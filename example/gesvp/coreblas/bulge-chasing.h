 ...
 **/
#ifndef _EIG_BULGE_H_
#define _EIG_BULGE_H_
 ...
 **/
inline static void eig_findVTpos(int N, int NB, int Vblksiz, int sweep, int st, int *Vpos, int *TAUpos, int *Tpos, int *myblkid);
inline static void eig_findVTsiz(int N, int NB, int Vblksiz, int *blkcnt, int *LDV);
inline static int eig_plasma_ceildiv(int a, int b);
...
inline static void eig_findVTpos(int N, int NB, int Vblksiz, int sweep, int st, int *Vpos, int *TAUpos, int *Tpos, int *myblkid)
{
  ...
  for (prevcolblkid = 0; prevcolblkid < nbprevcolblk; prevcolblkid++)
  {
       ...
       prevcolblknb = eig_plasma_ceildiv((N-(mastersweep+2)),NB);
       prevblkcnt   = prevblkcnt + prevcolblknb;
  }
  curcolblknb = eig_plasma_ceildiv((st-sweep),NB);
...
inline static void eig_findVTsiz(int N, int NB, int Vblksiz, int *blkcnt, int *LDV)
{
  ...
  *blkcnt   = 0;
  nbcolblk = eig_plasma_ceildiv((N-1),Vblksiz);
  for (colblk = 0; colblk<nbcolblk; colblk++)
  {
        mastersweep = colblk * Vblksiz;
        curcolblknb = eig_plasma_ceildiv((N-(mastersweep+2)),NB);
