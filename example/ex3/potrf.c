...
int AL4SAN_gemm(double alpha, AL4SAN_desc_t *A, CHAM_desc_t *B, double beta, CHAM_desc_t *C )
{
    AL4SAN_context_t *al4sanctxt;
    AL4SAN_sequence_t *sequence = NULL;
    AL4SAN_request_t* request = RUNTIME_REQUEST_INITIALIZER;
   ...
   AL4SAN_Options_Init(&options, sequence, request);
    ...
    for (m = 0; m < C->mt; m++) {
        ...
        for (n = 0; n < C->nt; n++) {
           ...
           AL4SAN_Data_Flush( sequence, C(m, n) );
           }
            for (k = 0; k < A->nt; k++) {
...
int main(int argc, char* argv[]){
    ...
    AL4SAN_desc_t *descAG = NULL,  *descCG = NULL, *descBG = NULL;
...
*/
   ...
   AL4SAN_Desc_Discharge( &descA );
   AL4SAN_Desc_Discharge( &descB );
   AL4SAN_Desc_Discharge( &descX );
   AL4SAN_Desc_Discharge( &descAC );

   AL4SAN_Switch_Runtime("Quark", NCPU, 0);

    AL4SAN_Desc_Create(&descAG,  descA->mat, Al4sanRealDouble,
                      NB, NB,  NB*NB, N, N, 0, 0, N, N, 1, 1);
    AL4SAN_Desc_Create(&descBG,  descB->mat, Al4sanRealDouble,
                      NB, NB,  NB*NB, N, NRHS, 0, 0, N, NRHS, 1, 1);
    AL4SAN_Desc_Create(&descCG,  descB->mat, Al4sanRealDouble,
                      NB, NB,  NB*NB, N, NRHS, 0, 0, N, NRHS, 1, 1);

     double alpha = 0.1, beta=-0.1;
    AL4SAN_gemm(alpha, descAG, descBG, beta, descCG);
   //   fflush("stdout");
   AL4SAN_Switch_Runtime("Starpu", NCPU, NGPU);
