...
CRITICAL_SECTION altanalwinthread_static_initializer_check_lock;
static int altanalwinthread_initialized = 0;
...
static int pthread_mutex_check_for_static_initialization( pthread_mutex_t *mutex ) {
    ...
    if ( altanalwinthread_initialized == 0 ) {
        InitializeCriticalSection( &altanalwinthread_static_initializer_check_lock );
        altanalwinthread_initialized = 1;
    }
    EnterCriticalSection( &altanalwinthread_static_initializer_check_lock );
    if ( *mutex == PTHREAD_MUTEX_INITIALIZER )
        retval = pthread_mutex_init( mutex, NULL );
    LeaveCriticalSection( &altanalwinthread_static_initializer_check_lock );
    return retval;
}
...
void *(*ALTANAL_realThStart)(void *);

/*
  This function is only called to have a proxy that is compatible with WINAPI.
 */
unsigned WINAPI ALTANAL_winThStart(void *arg) {
  ALTANAL_realThStart( arg );
  return 0;
}

ALTANAL_DLLPORT int ALTANAL_CDECL pthread_create(pthread_t *thread, const pthread_attr_t *attr, void *(*start) (void *), void *arg) {
...
ALTANAL_DLLPORT int ALTANAL_CDECL pthread_setconcurrency (int level) {
