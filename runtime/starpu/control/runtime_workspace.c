...
static void AL4SAN_Runtime_allocate_workspace_on_workers(void *arg)
{
    struct al4san_runtime_ws_s *workspace = arg;
...
static void AL4SAN_Runtime_free_workspace_on_workers(void *arg)
{
    struct al4san_runtime_ws_s *workspace = arg;
    enum starpu_worker_archtype type = 0;
    (void)type;
 ...
 */
int AL4SAN_Runtime_starpu_ws_alloc(AL4SAN_starpu_ws_t **workspace,
                            size_t size, int which_workers, int memory_location)
{
    if (!workspace)
        return -EINVAL;

    struct al4san_runtime_ws_s *descr = calloc(1, sizeof(struct al4san_runtime_ws_s));
    ...
    *workspace = descr;
    ...
    starpu_execute_on_each_worker(AL4SAN_Runtime_allocate_workspace_on_workers, descr, which_workers);
...
int AL4SAN_Runtime_starpu_ws_free(AL4SAN_starpu_ws_t *workspace)
{
    ...
    starpu_execute_on_each_worker(AL4SAN_Runtime_free_workspace_on_workers, workspace, workspace->which_workers);
...
{
    struct al4san_runtime_ws_s *descr = workspace;
    int id = starpu_worker_get_id();
