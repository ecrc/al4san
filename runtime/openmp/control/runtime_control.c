                  ...
                  int nthreads_per_worker)
{
    int hres = 0;
    if ( ncudas > 0 )
        al4san_warning( "AL4SAN_Runtime_init_scheduler(OpenMP)", "GPUs are not supported for now");

    if ( nthreads_per_worker > 0 )
        al4san_warning( "AL4SAN_Runtime_init_scheduler(OpenMP)", "Multi-threaded kernels are not supported for now");
...
void AL4SAN_Runtime_barrier_on(AL4SAN_context_t *al4san, double *ptr)
{
    #pragma omp barrier
    (void)al4san;
}
