...
#define AL4SAN_UNDEFINED_MASK 0x0000FF
...
typedef enum { AL4SAN_PRIORITY=AL4SAN_UNDEFINED,
...
#define AL4SAN_ASSERT_MSG(x, msg, ...) do {\
        if(!(x))\
        {\
...
#define AL4SAN_REALLOC(ptr, size) do { void *_new_ptr = realloc(ptr, size); AL4SAN_ASSERT_MSG(_new_ptr != NULL, "Cannot reallocate %ld bytes\n", (long) (size)); ptr=_new_ptr;} while (0)
...
int OPENMP_##name##_func (kmp_int32 gtid, kmp_task_t *task) { AL4SAN_arg_list al4san_arg; al4san_arg.task=task; cpu_func_name(&al4san_arg); return 0; }
...
void cpu_func_name(AL4SAN_arg_list *al4san_arg); \
int OPENMP_##name##_func (kmp_int32 gtid, kmp_task_t *task) { AL4SAN_arg_list al4san_arg; al4san_arg.task=task; cpu_func_name(&al4san_arg); al4san_workspace_destroy(&al4san_arg); return 0; }
