* Enabling auto-completion for altanal in spacemacs
...
** Installing Altanal
#+begin_src sh :results output :exports both
export ALTANAL_DIR=/path/to/altanal-src/
spack install -v altanal~simu@src ^mkl
#+end_src
** Loading modules
#+begin_src sh :results output :exports both
module purge
spack load altanal ; spack load cmake ; spack load fxt ; spack load openmpi ; spack load hwloc ; spack load starpu
#+end_src
** Building altanal with cc_args.py
#+begin_src sh :results output :exports both
cd $ALTANAL_DIR
